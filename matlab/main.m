...
imwrite(uint8(InputEdges), sprintf('%s_pd_%3.2f_pf_%3.2f_input.gif', ImageFile, pdd, pff), 'GIF');

% Save the traditional vision edges as an image
TIOutput = DrawSelectedLines (Lines, Layout, TOutput);
imwrite(uint8(TIOutput), sprintf('%s_pd_%3.2f_pf_%3.2f_software.gif', ImageFile, pdd, pff), 'GIF');

% Save the magnet vision edges as an image
MIOutput = DrawSelectedLines (Lines, Layout, MCoupled>ShowPerfThreshold );
imwrite(uint8(MIOutput), sprintf('%s_pd_%3.2f_pf_%3.2f_magnetic.gif', ImageFile, pdd, pff), 'GIF');
...
subplot(1,4,1); plot (Pf(ceil(ShowPerfThreshold *100)), Pd(ceil(ShowPerfThreshold *100)), 'ro'); 
 ...
 OFile = sprintf('%s_AllOutput.mat', ImageFile);
